...
for (i in 1:sims) {
  ...
  pts_sim <- pts_week %>% 
    ...
    left_join(pp_week[, c('roster_id', 'gameweek', 'pp', 'win')], by = c('roster_id', 'gameweek'), suffix = c('', '_pp')) %>% 
    mutate(roster_id = as.numeric(roster_id),
           ...
           pp_sim = if_else(gameweek>gameweek_end, pts_sim*pp_uplift, pp),
           pp_win_sim = if_else(gameweek>gameweek_end, win_sim, win_pp),
  ...
  table_sim <- pts_sim %>% 
    ...
    arrange(Playoff, PWins, PPoints) %>% 
    mutate(Draft_Pos = seq.int(nrow(.))) %>% 
    ungroup
...
The dots on the chart below show how many wins the team have, whilst the arrow shows where they should actually be if the H2H format wasn't in place. Teams with arrows pointing left can count themselves unlucky, whilst teams with arrows pointing right are fortunate to have as many wins as they do.
...
``` {r draft position probability, echo=F, message=F}
