---
title: "2023 NFL Dynasty League - Week 13"
...
KTC$Name_Team <- KTC_dynasty_url %>% html_nodes(xpath = '//*[@id="rankings-page-rankings"]/div/div/div/div[2]') %>% html_text %>% gsub('\t', '', .) %>% gsub('\n', '', .) %>% gsub('\r', '', .) %>% str_trim() %>% 
...
scoring_history <- ffscrapr::ff_scoringhistory(dynasty_conn, season = 2013:2022)
...
```{r roster comparison dynasty, echo = F, message = F, eval = F}
...
div(transactions %>% 
  ...
  layout(title = ' Number of 2023 Trades by Team',
