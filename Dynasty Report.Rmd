...
df_KTC_join <- df_KTC %>% 
  ...
  mutate(Team = case_when(Team=='KCC' ~ 'KC',
         ...
         Team = case_when(Player=='Will Fuller' ~ NA_character_,
                          ...
                          Player=='Justin Jackson' ~ NA_character_,
                          Player=='Jerick McKinnon' ~ NA_character_,
                          Player=="Ty'Son Williams" ~ 'BAL',
                          Player=="Lil'Jordan Humphrey" ~ NA_character_,
         ...
         Player = case_when(Player=='D.J. Moore' ~ 'DJ Moore',
                            Player=='Kenneth Walker' ~ 'Ken Walker',
...
for (i in 1:league_size) {
  df<-data.frame(t(unlist(rostersParsed[i]))) %>%
    ...
    pivot_longer(cols = starts_with('player'), names_to = 'player_no', values_to = 'player_id') %>%
...
dynasty_roster_value <- rosters %>% 
  left_join(df_KTC_join[, c('player_id', 'Value', 'Position')], by = 'player_id') %>% 
  filter(!is.na(Value)) %>%
  group_by(Manager) %>%
...
fantasy_roster_value <- rosters %>% 
  left_join(df_KTC_fantasy_join[, c('player_id', 'Value', 'Position')], by = 'player_id') %>% 
  filter(!is.na(Value)) %>%
  group_by(Manager) %>%
  summarise(Value = sum(Value, na.rm = T)) %>% 
  arrange(desc(Value)) %>% 
